...
def get_token():
    payload = {
        "staff": {
            ...
            "account": {"account_id": 1, "account_type": "manager"},
        },
        "iss": ISSUER,
    }
...
def login():
    try:
        ...
        if staff is not None:
            payload = {"staff": staff, "iss": ISSUER}
            token = jwt.encode(payload, SECRET_KEY, algorithm="HS256")
            payload["token"] = token
            ...
            return (
                jsonify({"success": False, "message": "Invalid staff ID or password"}),
                401,
            )
...
def create_shift():
    ...
    try:
        ...
        if start_time is None or end_time is None or staff_ids is None:
            return (
                jsonify({"success": False, "message": "Missing shift information"}),
                404,
            )
...
def set_clock_out():
    ...
    try:
        ...
        record = ShiftRecord(
            shift_record_id=shift_record_id, staff_id=staff_id, check_out=check_out
        )
...
def get_this_week_shift_records():
    ...
    try:
        ...
        return jsonify({"success": True, "shift_records": shift_records}), 200
    except Exception as e:
        print(f"Error: {e}")
        return jsonify({"success": False, "message": "Internal server error"}), 500
...
def create_payroll():
    ...
    try:
        ...
        if id is not None:
            return jsonify({"success": True, "payroll_id": id}), 200
        else:
            return (
                jsonify(
                    {
...
def calculate_salary():
    ...
    try:
        ...
        records = record_db.get_shift_records_in_range(
...
def create_communication():
    ...
    if data.get("type") == "announcement":
        ...
        try:
            ...
            if id is not None:
                return (
                    jsonify({"success": True, "communication_id": id, "type": "mail"}),
                    200,
                )
...
def update_password():
    ...
    try:
        ...
        ok = db.update_password(
            staff_id=staff_id, old_password=old_password, new_password=new_password
        )
        if ok:
            return (
                jsonify(
            ...
            return jsonify({"success": False, "not_match": True}), 200
