...
class ShiftsDatabase:
    ...
    def get_shift_by_id(self, id: int) -> Optional[dict]:
        """
        @brief Retrieve single shift by shift_id.
            This method ensure MongoDB connection
            -> query shifts collection for document with given shift_id.
            If found,
            -> resolve staffs field into full staff document using StaffsDatabase
            -> convert internal Mongo ObjectId to string for JSON
            -> return shift document

        @param id Int id of shift for retrieval.
        @return Dictionary representing shift document with resolved shift info and stringified id or None if not found or error.
    ...
    def get_active_shifts(self) -> Optional[list[dict]]:
        ...
        try:
            ...
            query = {
                "$and": [{"start_time": {"$lte": now}}, {"end_time": {"$gt": now}}]
            }
    ...
    def get_done_shifts(self) -> Optional[list[dict]]:
        """
        @brief Retrieve all complete shifts.
            Shift is considered done if end_time ealier than current time.
            This method retruns a simplified list of done shifts containing only shift_id, start_time and end_time (time format ISO string).

        @return  Listof dictionaries for completed shifts or None of no completed shifts or error.
    ...
    def insert_shift(self, shift: Shift) -> Optional[str]:
        """
        @brief Insert new shift document into database.
            This method
            -> generate unique random shift_id
            -> convert start and end time from shift domain object into datetime instances
            -> create new shift document in shifts collection.

        @param shift Shift fomain object containing information to be inserted.
        @return Generated shift_id if insert successful or None if error.
    ...
    def delete_shift(self, shift_id: int) -> bool:
        ...
        @brief Delete shift document by shift_id.
            This method delete single shift document that match the shift_id.

        @param shift_id Int id of shift to be deleted.
        @return True if document deleted, else False or if error.
